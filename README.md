...
└── .claude/
    ...
    └── skills/
        ├── example-code-reviewer.md  # コードレビュースキルの例
        └── example-test-writer.md    # テスト作成スキルの例
```
...
Claude Code がサブエージェントとして呼び出せる、プロジェクト固有のスキルを定義するファイルです。  
各スキルは **Markdown ファイル 1 つ = スキル 1 つ** に対応します。

セットアップ後、以下の 2 つのサンプルが配置されます。

| ファイル | 説明 |
|---------|------|
| `example-code-reviewer.md` | コードレビューを行うスキルのサンプル |
| `example-test-writer.md`   | テストコードを作成するスキルのサンプル |
...
# model: claude-opus-4-5  # 使用モデル（省略時は親セッションを引き継ぐ）
---
